...
// Проверяет, является ли символ пробельным (по ASCII): пробел, таб, перевод строки и т.п.
...
// Приводит ASCII-букву к нижнему регистру; остальные символы возвращает без изменений.
...
// Смотрит текущий байт входа без потребления: записывает его в out и возвращает 1; если конец строки — 0.
...
// Потребляет один байт входа: записывает его в out, сдвигает курсор и увеличивает счётчик consumed; на '\0' ставит input_fail.
...
// Пропускает (потребляет) все пробельные символы во входе, пока они встречаются подряд.
...
// Потребляет символы, пока pred(c) истинно и не превышен лимит limit; возвращает количество потреблённых байтов.
