...
// Возвращает эффективную ширину для float-разбора: если width не задана,
// считаем её бесконечной.
static int s21_effective_width_float(int width);

// Откатывает входной контекст к сохранённой позиции (используется при
// mismatch/невалидной экспоненте и т.п.).
static void s21_restore_ctx_f(s21_scan_ctx_t *ctx, const s21_fstate_t *st);

// Проверяет, можно ли читать ещё один символ из входа с учётом оставшейся
// ширины.
static int s21_can_read_f(const s21_scan_ctx_t *ctx, int wleft);

// Возвращает текущий символ входа без потребления (0 при некорректном
// контексте).
static unsigned char s21_peek_f(const s21_scan_ctx_t *ctx);

// Потребляет один символ входа: двигает курсор, увеличивает consumed и
// уменьшает wleft.
static void s21_consume_f(s21_scan_ctx_t *ctx, int *wleft);

// Сопоставляет ключевое слово kw с входом без учёта регистра в пределах wleft;
// возвращает длину совпадения.
static int s21_match_ci_kw(const char *in, int wleft, const char *kw,
                           int *klen);

// Пытается разобрать специальные значения nan/inf/infinity (без учёта
// регистра); при успехе пишет *out.
static int s21_try_parse_special(s21_scan_ctx_t *ctx, s21_fstate_t *st,
                                 long double *out);

// Читает опциональный знак '+'/'-' и сохраняет его в st->sign.
static void s21_parse_sign_f(s21_scan_ctx_t *ctx, s21_fstate_t *st);

// Читает последовательность десятичных цифр (до переполнения ULLONG_MAX с
// насыщением), возвращает значение и количество цифр.
static unsigned long long s21_parse_digits_u64(s21_scan_ctx_t *ctx, int *wleft,
                                               int *nd);

// Возвращает 10^exp в формате long double (для масштаба дробной части и
// экспоненты).
static long double s21_pow10_ld(int exp);

// Пытается разобрать экспоненту вида e[+/-]digits; при неудаче откатывает
// позицию и ширину.
static int s21_try_parse_exponent(s21_scan_ctx_t *ctx, int *wleft, int *exp10);

// Разбирает обычное число с плавающей точкой (не nan/inf): целая часть, дробная
// часть, экспонента.
static s21_scan_status_t s21_read_float_regular(s21_scan_ctx_t *ctx,
                                                s21_fstate_t *st,
                                                long double *out);

// Ядро разбора float по правилам scanf: [sign] (digits[.digits] | .digits) [e/E
// [sign] digits], плюс nan/inf. Возвращает статус (OK / MATCH_FAIL /
// INPUT_FAIL) и при успехе пишет значение в *out.
...
// Обёртка для спецификаторов %f/%e/%g (и их варианты с длиной): пропускает
// пробелы, читает число и пишет его в аргумент (если не подавлено '*').
s21_scan_status_t s21_scan_float(s21_scan_ctx_t *ctx,
...
// Проверяет возможность чтения: ctx валиден, ширина не исчерпана и не достигнут
// '\0'.
static int s21_can_read_f(const s21_scan_ctx_t *ctx, int wleft) {
  int ok = 0;
...
static void s21_consume_f(s21_scan_ctx_t *ctx, int *wleft) {
  if (ctx != NULL && wleft != NULL && *wleft > 0 && ctx->in != NULL &&
      *ctx->in != '\0') {
    ctx->in++;
    ctx->consumed++;
    (*wleft)--;
...
// Разбирает и запоминает знак числа; знак не считается "цифрой", но влияет на
// итоговое значение.
static void s21_parse_sign_f(s21_scan_ctx_t *ctx, s21_fstate_t *st) {
...
// Вычисляет 10^exp через умножение/деление на крупные ступени
// (1e16/1e8/1e4/10), чтобы не гонять циклы по одной степени.
static long double s21_pow10_ld(int exp) {
  long double r = 1.0L;
  int e = exp;
  int neg = 0;
...
// Пытается разобрать экспоненту: 'e'/'E' затем необязательный знак и хотя бы
// одна цифра. Если после 'e' не нашлось цифр, откатывает ctx и wleft назад, как
// будто экспоненты не было.
static int s21_try_parse_exponent(s21_scan_ctx_t *ctx, int *wleft, int *exp10) {
  int ok = 0;
...
// Сравнивает вход с ключевым словом без учёта регистра в пределах wleft; на
// успех возвращает длину совпадения.
static int s21_match_ci_kw(const char *in, int wleft, const char *kw,
                           int *klen) {
  int ok = 0;
  int i = 0;
...
// Пытается разобрать специальные значения: nan, nan(...), inf, infinity (без
// учёта регистра). При успехе потребляет соответствующие символы из входа и
// выставляет *out.
static int s21_try_parse_special(s21_scan_ctx_t *ctx, s21_fstate_t *st,
                                 long double *out) {
  int ok = 0;
  ...
  if (ctx != NULL && st != NULL && out != NULL) {
    ...
    if (s21_match_ci_kw(ctx->in, st->wleft, "nan", &klen)) {
      ...
      *out = NAN;
      ...
      if (s21_can_read_f(ctx, st->wleft) &&
          s21_peek_f(ctx) == (unsigned char)'(') {
        s21_consume_f(ctx, &st->wleft);
        while (s21_can_read_f(ctx, st->wleft) &&
               s21_peek_f(ctx) != (unsigned char)')') {
          s21_consume_f(ctx, &st->wleft);
        }
        if (s21_can_read_f(ctx, st->wleft) &&
