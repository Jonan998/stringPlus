...
/*
файл отвечает за разбор строки формата sscanf — превращает %... в структуру токена (подавление *, ширина, модификатор длины, спецификатор и scanset %[...])
*/

// Разбирает символ подавления присваивания '*' после '%'.
static const char *s21_parse_suppress(const char *p, s21_fmt_token_t *t);

// Разбирает ширину поля (последовательность цифр) после '*' и до модификатора длины/спецификатора.
...
// Строит таблицу scanset для %[...], двигает указатель формата за закрывающую ']' и выставляет инверсию '^'.
static int s21_fill_scanset(unsigned char set[S21_SSCANF_SET_SIZE],
                            const char **pp, int *invert);

// Инициализирует токен формата значениями по умолчанию и обнуляет таблицу scanset.
...
// Разбирает одну спецификацию преобразования, начинающуюся с '%' (флаги, ширина, длина, тип, включая %[...]).
// Возвращает указатель на следующий символ формата после спецификатора, либо fmt при ошибке парсинга.
...
// Разбирает ширину поля (целое число) и записывает её в токен; при переполнении насыщает до INT_MAX.
static const char *s21_parse_width(const char *p, s21_fmt_token_t *t) {
  const char *q = p;
  int width = 0;
  int has_digits = 0;
...
// Разбирает модификатор длины (hh/h/l/ll/L) и сдвигает указатель формата дальше.
static const char *s21_parse_length(const char *p, s21_fmt_token_t *t) {
  const char *q = p;
...
// Проверяет, является ли текущий символ '-' частью диапазона в scanset (например, a-z), а не литералом '-'.
...
// Отмечает диапазон символов [a..b] как входящий в scanset (если a > b, диапазон разворачивается).
