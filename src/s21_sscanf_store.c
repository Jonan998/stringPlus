...
/*
назначение файла: записывать распарсенные значения в переменные пользователя
через va_list конвертеры читают и вычисляют значение, а эти функции отвечают за
корректный тип (hh/h/l/ll/L) и безопасную запись по указателю
*/

// Записывает знаковое целое значение в переменную нужного типа в зависимости от
// модификатора длины.
...
// Записывает беззнаковое целое значение в переменную нужного типа в зависимости
// от модификатора длины.
...
// Записывает результат %p: кладёт указатель p в переменную пользователя
// (ожидается void **)
...
// Записывает число с плавающей точкой в зависимости от модификатора длины:
...
// Реализация %n: записывает количество уже потреблённых символов входа в
