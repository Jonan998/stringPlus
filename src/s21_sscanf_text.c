...
/*
отвечает за разбор и чтение текстовых спецификаторов sscanf - %s, %c и %[...]
(scanset)
*/
...
// Реализует спецификатор %s: пропускает пробелы, читает непробельную
// последовательность с учётом width и пишет в буфер (если не подавлено '*').
...
// Реализует спецификатор %c: читает ровно width символов (по умолчанию 1),
// пробелы не пропускает; при успехе пишет байты в буфер
...
// Реализует спецификатор %[...]: читает символы, входящие (или не входящие при
// '^') в набор, с учётом width; при успехе пишет строку и завершающий '\0'.
...
// Возвращает эффективную ширину для текстовых спецификаторов: если width не
// задана, подставляет defv.
static int s21_effective_width_text(int width, int defv) {
  int w = width;
  if (w <= 0) w = defv;
  return w;
...
// Проверяет, принимает ли scanset символ "c": учитывает таблицу t->set и
// инверсию '^'.
static int s21_scanset_accept(const s21_fmt_token_t *t, unsigned char c) {
  int ok = 0;
...
// Пишет один байт в буфер по индексу idx, если запись разрешена (не подавлено
// '*' и dst не NULL).
static void s21_write_byte(char *dst, int do_write, int idx, unsigned char c) {
