...
static void s21_restore_ctx(s21_scan_ctx_t *ctx, const char *in0,
                            int consumed0);
...
static void s21_read_optional_sign(s21_scan_ctx_t *ctx, int *wleft,
                                   int allow_sign, int *sign);
static int s21_detect_base_auto(const s21_scan_ctx_t *ctx, int wleft);
static void s21_consume_hex_prefix_if_valid(s21_scan_ctx_t *ctx, int *wleft,
                                            int base);

/*
файл отвечает за разбор целочисленных и указательных преобразований sscanf
(ядро чтения числа с выбором базы/знаком/шириной и обёртки для
%d/%i/%u/%o/%x/%X/%p с последующей записью результата)
*/

// Ядро разбора целого числа: читает знак (опционально), определяет основание (в
// т.ч. AUTO для %i), затем потребляет цифры с учётом ширины и накапливает
// модуль в unsigned long long.
  ...
  if (ctx == NULL || ctx->in == NULL || mag == NULL || sign == NULL) {
    ...
    *sign = 1;
    ...
    if (!s21_can_read(ctx, wleft)) {
      ...
      base = (base_mode == S21_BASE_AUTO) ? s21_detect_base_auto(ctx, wleft)
                                          : base;
...
// Обрабатывает знаковые целые спецификаторы (например %d и %i): вызывает ядро
// разбора, применяет знак и насыщение, затем при необходимости пишет результат
// в аргумент пользователя с учётом модификатора длины.
  ...
  if (ctx == NULL || t == NULL) {
    ...
    if (res == S21_SCAN_OK) {
      if (sgn < 0) {
        ...
        outv =
            (mag > (unsigned long long)LLONG_MAX) ? LLONG_MAX : (long long)mag;
      }
...
// Обрабатывает беззнаковые целые спецификаторы (например %u/%o/%x/%X): читает
// модуль (и возможный знак), затем при необходимости пишет результат в аргумент
// пользователя с учётом модификатора длины.
s21_scan_status_t s21_scan_unsigned(s21_scan_ctx_t *ctx,
  ...
  if (ctx == NULL || t == NULL) {
    ...
    res =
        s21_read_int_core(ctx, t->width, (s21_base_mode_t)base, 1, &mag, &sgn);
    if (res == S21_SCAN_OK) {
...
// Обрабатывает %p: читает шестнадцатеричное значение (опционально с 0x при
// наличии валидных цифр), затем при необходимости пишет указатель в аргумент
// пользователя.
...
// Преобразует один символ в значение цифры: 0..15 для 0-9/a-f/A-F, либо -1 если
// символ не цифра.
static int s21_digit_val(unsigned char c) {
  int v = -1;
...
// Возвращает эффективную ширину: если width не задана (<=0), то считаем её
// бесконечной (INT_MAX).
static int s21_effective_width(int width) {
  int w = width;
  if (w <= 0) w = INT_MAX;
...
// Откатывает контекст ввода на сохранённые значения (нужно при mismatch, когда
// ничего не прочитали).
static void s21_restore_ctx(s21_scan_ctx_t *ctx, const char *in0,
                            int consumed0) {
  if (ctx != NULL) {
...
// Проверяет, можно ли читать следующий символ: есть ширина, не конец строки и
// валидный ctx.
static int s21_can_read(const s21_scan_ctx_t *ctx, int wleft) {
  int ok = 0;
...
// Возвращает текущий символ входа как int без потребления (0, если ctx
// некорректен).
static int s21_peek_char(const s21_scan_ctx_t *ctx) {
  int c = 0;
...
// Накапливает очередную цифру в acc в заданной базе; при переполнении насыщает
// до ULLONG_MAX.
static void s21_accumulate_ull(unsigned long long *acc, int digit, int base) {
  if (acc != NULL) {
...
// Читает необязательный знак '+'/'-' (если разрешено allow_sign) и обновляет
// sign и оставшуюся ширину.
static void s21_read_optional_sign(s21_scan_ctx_t *ctx, int *wleft,
                                   int allow_sign, int *sign) {
  if (ctx != NULL && wleft != NULL && sign != NULL && allow_sign &&
      s21_can_read(ctx, *wleft)) {
...
// В режиме AUTO (%i) выбирает базу: по ведущему '0' => 8, по "0x" + валидной
// hex-цифре => 16, иначе 10.
static int s21_detect_base_auto(const s21_scan_ctx_t *ctx, int wleft) {
  int base = 10;

  if (ctx != NULL && ctx->in != NULL && wleft > 0 &&
      (unsigned char)ctx->in[0] == (unsigned char)'0') {
    base = 8;
    if (wleft >= 3) {
      unsigned char c1 = (unsigned char)ctx->in[1];
      unsigned char c2 = (unsigned char)ctx->in[2];
      if ((c1 == (unsigned char)'x' || c1 == (unsigned char)'X') &&
          s21_is_xdigit(c2)) {
        base = 16;
      }
    }
...
// Для базы 16 потребляет префикс "0x"/"0X", но только если после него реально
// идёт hex-цифра (и хватает ширины).
static void s21_consume_hex_prefix_if_valid(s21_scan_ctx_t *ctx, int *wleft,
                                            int base) {
  if (ctx != NULL && wleft != NULL && base == 16 && ctx->in != NULL &&
      *wleft >= 3) {
    ...
    if (c0 == (unsigned char)'0' &&
        (c1 == (unsigned char)'x' || c1 == (unsigned char)'X') &&
        s21_is_xdigit(c2)) {
      unsigned char tmp = 0U;
      (void)s21_take_in(ctx, &tmp);
      (void)s21_take_in(ctx, &tmp);
